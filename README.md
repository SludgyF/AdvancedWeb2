# **Tarea 2 (Advanced Web) (Adolfo)**

# **Advanced Web Tarea 2**

## Equipo #46

Carlos Iván Armenta Naranjo - A01643070

Jorge Javier Blásquez Gonzalez - A01637706 

Adolfo Hernández Signoret - A01637184

Arturo Ramos Martínez - A01643269

Moisés Adrián Cortés Ramos - A01642492

Bryan Ithan Landín Lara - A01636271

## 🚀 To run use:

Para poder probarlo ademas se necesita la Wallet, por cuestiones de seguridad no esta subida en el repositorio

```bash
git clone https://github.com/SludgyF/AdvancedWeb2.git
```
```
cd .\AdvancedWeb2\LibraryManagement\
```
```
mvn clean install
```
```
mvn spring-boot:run
```
# **Acceder a la documentacion**
http://localhost:8080/swagger-ui/index.html

# **Pruebas de la documentacion**

Dashboard de Swagger
![Dashboard de Swagger 1](proof-images/image.png)

Dashboard de Swagger
![Dashboard de Swagger 2](proof-images/swaggerdash2.png)

Inicio de sesion para obtener token
![Inicio de sesion para obtener token](proof-images/singin1.png)

Obtenemos token
![Obtenemos token](proof-images/sigin2.png)

Autorizamos con token
![Autorizamos con token](proof-images/auth.png)

Se confirma la autorizacion
![Se confirma la autorizacion](proof-images/auth2.png)

Usamos endpoint de obtener todos los libros
![Usamos endpoint de obtener todos los libros](proof-images/books.png)

Usamos endpoint de obtener un libro por ID
![Usamos endpoint de obtener un libro por ID](proof-images/book.png)
